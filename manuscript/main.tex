...
Crucially, as explicitly demonstrated in recent advanced architectures, performing multi-qubit logic gates linking distant processors \cite{noiri_shuttling-based_2022} or executing two-qubit logic directly on mobile spins \cite{matsumoto_two-qubit_2025} is essential. To maintain fault-tolerant thresholds during these operations, especially when utilizing high-fidelity exchange control in industrial platforms \cite{chittock-wood_radio-frequency_2024}, the relative phase coherence must be strictly preserved across the entire dynamic transport trajectory. 

Despite these strict requirements, the traditional assumption regarding shuttling-induced dynamical phases—that they can be treated as independent, local single-qubit rotations \cite{de_smet_high-fidelity_2025}—ceases to hold in dense two-dimensional architectures. When multiple qubits are simultaneously shuttled, the spatially structured control fields, electrostatic crosstalk, and anisotropic g-tensor effects \cite{geyer_anisotropic_2024, fedele_simultaneous_2021} induce correlated non-local phase shifts. This forms a dense, deterministic phase network coupled across all moving qubits, constituting a spatiotemporally correlated error channel fundamentally distinct from conventional localized stochastic noise \cite{cifuentes_impact_2024, jin_probing_2024, blume-kohout_demonstration_2017, wilen_correlated_2021}. 

From an architectural perspective, this spatial correlation imposes a severe scalability constraint. Unlike local phase errors, correlated accumulation cannot be mitigated by static compensation techniques \cite{tanttu_assessment_2024}. As the number of simultaneously shuttled qubits $K$ increases, the dimensionality of the relative phase calibration matrix exhibits a quadratic scaling overhead ($O(K^2)$). Because multi-qubit logic strictly depends on the relative computational phases, this dimensionality bottleneck renders conventional independent-error calibration and analytical optimal control experimentally impractical \cite{proctor_benchmarking_2025, glaser_training_2015}, inevitably leading to a severe degradation of logical fidelity at the system level. 

To overcome this architectural barrier, we propose a multiplexed inference and software-level compensation strategy. Instead of attempting to physically eliminate all correlated phases through increasingly isolated hardware, our framework (Fig.~\ref{fig:figure1}) treats the deterministic phase network as a learnable susceptibility operator. Multiplexed Ramsey-based system identification \cite{ruffino_cryo-cmos_2022, harper_efficient_2020} acquires parallel phase responses to randomized displacement probes, while a physics-informed decomposition organizes the recovered operator into a geometric $1/r^3$ baseline and a residual device-specific correction. The resulting surrogate is then used by the compiler to apply virtual Z-rotations, compensating transport-induced relative phases without adding physical microwave operations.
...
Furthermore, while our microscopic derivations are explicitly framed around electron spins navigating nanomagnet stray fields, the emergent $O(K^2)$ dimensionality bottleneck and the proposed inference-control framework extend naturally to a broad class of mobile solid-state qubit platforms governed by spatial frequency gradients. This includes emerging architectures such as germanium hole-spin qubits, where active routing through strong intrinsic spin-orbit coupling (SOC) landscapes produces closely analogous, highly sensitive electrostatic crosstalk networks. In this broader perspective, deterministic relative phase tracking should be viewed not merely as a platform-specific calibration routine, but as an important architectural component for scalable solid-state quantum computing.
...
(c) Scalable compensation strategy via multiplexed inference. To address this bottleneck, multiplexed probing (bottom left) acquires parallel phase data ($\mathbf{D}_\theta$) from simultaneously active qubits via $m$-channel parallel control. An inference engine (top center) reconstructs the susceptibility operator and organizes it into a geometric baseline ($\mathbf M_{\rm prior}$, teal) and a data-driven residual correction operator ($\Delta\mathbf M_{\rm res}$, red). In the overdetermined regime studied here, this decomposition provides physical interpretability, while the reduction in calibration rounds comes from multiplexed probing. The inferred corrections are applied via virtual Z-gates (bottom right) at the compiler level, rotating the computational frame without additional physical control operations.
...
Crucially, the act of transporting multiple qubits in parallel introduces a new layer of strongly correlated crosstalk. The electromagnetic perturbations generated by shuttling operations propagate through the device and couple distant qubits in a nontrivial manner. This coupling leads to systematic phase fluctuations that are neither purely quantum-entangling nor purely local stochastic noise, but instead arise from the interplay between device-level electromagnetic response and coherent spin dynamics.
...
Crucially, as detailed in Sec.~\ref{sec:micro_source_electrostatic}, the transient electrostatic crosstalk generated by active routing inevitably displaces the electronic wavefunction within these extended magnetic gradients. In dense arrays, the long-range spatial decay (dipolar-like in nanomagnet-based devices) of these susceptibilities causes them to accumulate coherently across many simultaneously active control lines. Rather than acting as independent local noise, this collective perturbation forms a dense, non-sparse susceptibility network $\chi_{ij}$ across the processor (Fig.~\ref{fig:figure1}a,b).
Because every moving qubit potentially perturbs every other moving qubit, the dimensionality of this strongly coupled parameter space expands quadratically ($\mathcal{O}(K^2)$) with the active qubit count $K$. This un-truncatable long-range connectivity creates a fundamental structural bottleneck, rendering sequential pairwise calibration and analytical optimal control mathematically intractable and experimentally prohibitive beyond small arrays.
...
\end{equation}
inducing state-dependent phase shifts that cannot be captured by independent single-qubit models. However, unlike the long-range spatial crosstalk susceptibility $\chi_{ij}$ which scales algebraically and couples distant qubits, the exchange interaction is strictly short-range and exponentially suppressed with distance. Therefore, while $H_{ij}^{\text{ex}}$ requires careful calibration for nearest-neighbor routing, the long-range classical crosstalk (Sec.~\ref{sec:micro_source}) constitutes the dominant mechanism for system-level phase dispersion in large arrays.
...
The phase accumulated by a shuttled spin qubit is conventionally treated as a purely local quantity, perturbed only by independent stochastic noise environments. Under such assumptions, multi-qubit phase errors can be mitigated using statistically independent calibration protocols. However, Eq.~\eqref{eq:driven_phase} demonstrates that transport-induced phase errors are fundamentally driven by deterministic classical control signals. Consequently, these phase shifts do not behave as uncorrelated noise, but rather accumulate coherently across the device architecture.

In this section, we derive the mathematical bounds of this coherent phase accumulation and demonstrate why the spatial structure of the crosstalk fundamentally complicates conventional independent-error calibration strategies in large-scale arrays.
...
\end{equation}
Here, we have taken the absolute value of the susceptibility to obtain a strict worst-case upper bound. We note that this bound treats the envelope of the susceptibility as an isotropic scalar.

In realistic Si/SiGe and Ge-based planar spin qubit platforms, the dipolar magnetic field exhibits angular structure ($\propto (1-3\cos^2\theta)/r^3$), and the effective Larmor frequency further inherits anisotropic contributions from the g-tensor and strain-induced Stark modulation. These anisotropies can induce partial angular cancellations in specific geometric configurations. However, such angular dependencies modify only the prefactor of the spatial decay, not its leading algebraic exponent under dipole-dominated coupling. Therefore, while the numerical magnitude of the coherent accumulation may be reduced in optimized layouts, the geometric summability in two dimensions remains fundamentally unchanged.
...
In analogous scalable quantum platforms, such as neutral atom arrays, spatially correlated phase errors have emerged as a dominant obstacle, prompting the implementation of real-time feed-forward interventions via co-located auxiliary spectator qubits \cite{singh_mid-circuit_2023}. In contrast, the extremely dense hardware integration and continuous classical control fields intrinsic to planar semiconductor architectures introduce prohibitive routing and control overhead for the deployment of isolated spectator arrays. This physical constraint uniquely mandates a fundamentally different paradigm for spin architectures.
...
The severe dimensionality bottleneck ($O(K^2)$) highlighted in Sec.~\ref{sec:calibration_bottleneck} and computationally verified in Sec.~\ref{sec:numerical_bottleneck} dictates that scalable spin shuttling cannot rely on unstructured, empirical error mitigation. To overcome this fundamental limitation, the architecture requires a hardware-software co-design approach: the physical hardware must structurally bound the local phase dispersion and enable scalable multiplexed data acquisition, while the software compiler must interpret this data through a physically grounded model to deterministically track and compensate the residual macroscopic phase network.
...
While forward phase evaluation is computationally tractable given a predictive model, the inverse problem of actively shaping the transport trajectories to minimize relative phase dispersion is substantially more complex. During quantum logic operations, the shuttling velocity $v_i(t)$ is strictly bounded to preserve quantum information and avoid non-adiabatic orbital excitations. This operational constraint severely restricts the available kinematic control space. 

Furthermore, the optimization landscape becomes highly non-convex under these strict velocity and adiabaticity boundaries. The effective system Jacobian inherits the dense spatial coupling structure encoded in the geometric matrix $\mathbf{M}$, entangling all control directions. In large arrays, adjusting the trajectory of one qubit to minimize its local phase dispersion induces correlated phase shifts across the entire active register. 

Consequently, any hardware-in-the-loop search over transport trajectories would be expensive if it required repeated Ramsey evaluation of the full correlated response. The role of the inferred surrogate is to move this refinement offline: once $\hat{\mathbf M}$ is reconstructed, bounded trajectory adjustments can be tested computationally under the same phase-dispersion objective, while virtual-Z tracking remains the primary compensation mechanism used in the main scalability analysis.

\subsection{Architectural Implications for Scalable Spin-Shuttling Arrays}
\label{sec:architectural_implications}

The emergence of structured but analytically complex crosstalk necessitates a paradigm shift in how we define the fidelity of mobile qubit architectures. Traditionally, the fidelity of a quantum processor is decomposed into gate, readout, and routing components. However, our analysis reveals that \textit{phase compensation fidelity} constitutes a distinct and fundamental limiting factor.
...
\end{equation}
where the data matrix $\mathbf{D}_{\theta} \in \mathbb{R}^{n_{\text{exp}} \times K}$ collects one measured phase-response vector per row and $\mathbf{U} \in \mathbb{R}^{n_{\text{exp}} \times K}$ is the applied random probe matrix. The additive noise matrix $\mathbf{E}$ models the quantum projection noise, where each element $E_{ij}$ is independently drawn from a zero-mean Gaussian distribution with variance $\sigma_\varphi^2$. In this formulation, $\sigma_\varphi = 1/\sqrt{N_{\text{shots}}}$ denotes the fundamental phase noise scale defined by the measurement shot budget.
...
While the number of required experimental configurations is dramatically reduced to $O(K)$, extracting the true expectation phases from realistic hardware requires mitigating fundamental quantum projection noise ($\mathbf{E}$). Each configuration must be averaged over standard projection shots (typically $N_{\text{shots}} \sim 10^3$). Therefore, the total acquisition time scales as:
\begin{equation}
...
We first evaluate the fundamental scaling of the physical crosstalk network. We define the set of functionally relevant crosstalk pairs by the condition $|M_{ij}| > \varepsilon_{\text{FT}} / U_J$, corresponding to the maximum induced phase error exceeding the fault-tolerance threshold ($\varepsilon_{\text{FT}} = 10^{-3}$) under the characteristic displacement scale $U_J$. As previously demonstrated in Sec.~\ref{sec:numerical_bottleneck}, the number of non-negligible crosstalk pairs scales superlinearly ($\sim K^2$), confirming that practically all actively routed qubits fall within each other's effective interaction radii. This dense connectivity imposes a severe physical bottleneck.
...
Plain multiplexed least-squares regression already exposes the central statistical feature of the problem. Near the strict algebraic identifiability limit ($n_{\text{exp}}=K$), random probe matrices are often ill-conditioned, so finite shot noise is amplified by the pseudo-inverse. At the moderately overdetermined working point used throughout the main text ($n_{\text{exp}}=2K$), the same randomized design becomes sufficiently stable to support high-fidelity compensation in the adopted simulation model.
...
To properly contextualize this scalable inference, we apply the structured framework derived in Sec.~\ref{sec:physics_inference}. A crucial physical observation from our simulations is that subtracting the geometric prior ($\mathbf M_{\rm prior}$) does not significantly reduce the Frobenius norm of the parameter space ($||\Delta\mathbf{M}_{\text{res}}||_F / ||\mathbf{M}_{\text{prior}}||_F \approx 1.23$). 
...
While the preceding results demonstrate the framework within the adopted idealized parallel-probing model, near-term cryogenic platforms face strict wiring and readout bandwidth constraints. To address these key experimental constraints, we evaluate the calibration overhead under sub-array multiplexing protocols, where hardware limits the simultaneous probing capacity to $m$ qubits per experimental round. 
...
In this work, we have analyzed transport-induced correlated dephasing as a scalability bottleneck in dense spin-shuttling architectures. By reformulating shuttling-induced phase errors from a localized stochastic noise process into a deterministic, macroscopic spatial operator, we established that the calibration of relative computational phases suffers from a severe dimensionality constraint ($O(K^2)$). Unmitigated, this dense crosstalk network causes a rapid collapse of phase compensation fidelity as parallel routing density increases.
...
To realistically model fundamental quantum projection noise, simulated expectation phases are subjected to Gaussian noise with standard deviation $\sigma_\varphi = 1/\sqrt{N_{\text{shots}}}$, assuming a baseline budget of $N_{\text{shots}} = 1000$ typical measurement repetitions per experimental configuration.
...
Crucially, the identifiability of the inference problem depends on the rank and conditioning of the probe matrix $\mathbf{U}$. In the overdetermined regime ($n_{\text{exp}} > K$), the total susceptibility matrix is uniquely determined by the measurement data, provided that $\mathbf{U}$ has full column rank and is sufficiently well-conditioned. 
...
The data supporting the numerical figures are generated by the accompanying Python scripts and exported as the CSV source-data files listed above. For submission, the source-data CSV files and generated figure files will be deposited in a public archival repository such as Zenodo, with a versioned DOI. A placeholder repository structure is assumed during review: \url{https://github.com/USERNAME/control-space-scaling} and \url{https://doi.org/10.5281/zenodo.XXXXXXX}. These links should be replaced by the archived release and DOI before final submission.

\section*{Code Availability}
The simulation and figure-generation code consists of three standalone Python scripts: \path{simulation_stage1_final.py}, \path{simulation_stage2_final.py}, and \path{simulation_stage2_robustness.py}. The code uses NumPy, SciPy, and Matplotlib with double-precision arithmetic. The exact versioned code used to generate the submitted figures and CSV files will be deposited with the source-data package, together with an environment file and a short README describing how to regenerate each figure.
...
I.Y. conceived the project, developed the theoretical model, implemented the numerical simulations, analyzed the results, generated the figures, and wrote the manuscript.
...
Decomposing this frequency deviation into a strictly local contribution and a perturbative non-local crosstalk term yields:
\begin{equation}
...
\end{equation}
Expanding the voltage sensitivity, we arrive at the generalized fundamental expression:
\begin{equation}
...
(b) Compensation fidelity $F_{\rm comp}$ vs calibration experimental overhead $n_{\rm exp}/K$. Consistent with the mathematical equivalence in the overdetermined regime, both the plain multiplexed LS and the prior-assisted method exhibit identical trajectories, converging to $F_{\rm comp} > 0.999$ at $n_{\rm exp} \ge 2K$. Both methods suffer from condition-number-driven noise amplification near the algebraic limit ($n_{\rm exp} \to K$). Note: The scaling behavior depicted in panel (b) is fully consistent with Fig.~\ref{fig:summary2}(a) of the main text, provided here to contextually benchmark the element-wise scatter distribution shown in panel (a).}
